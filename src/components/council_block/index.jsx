...
const Promoter = ({ data, showDets = false }) => {
  ...
  return (
    <div className={styles.whitebox}>
      ...
      <div className="mt-3 mb-2">
        <h5 className={styles.thin}>{data.name}</h5>
        <div className="smaller mt-1">{data.description}</div>
      </div>
      {showDets && (
        <div className="d-flex mt-auto">
          ...
          <div className="my-auto ms-2">
            <div className={styles.smalltext}>{data.phone}</div>
          </div>
        </div>
      )}
      {showDets && (
        <div className="d-flex mt-0">
          ...
          <div className="my-auto ms-2">
            <div className={styles.smalltext}>{data.email}</div>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <Container className="my-5">
      <div className="d-flex">
        ...
        <div className="ms-auto my-auto">{/* FILTER */}</div>
      </div>
      ...
      <Row className="mt-4 mb-2">
        {loadedData.map((each, index) => {
          return (
            <Col data-aos="fade-up" md={6} lg={4} xl={3} key={index}>
              <Promoter data={each} />
            </Col>
          );
        })}
      ...
      <center>
        {hasMore && (
          <div onClick={loadMore} className={styles.morebutton}>
            {strings.more[locale]}
          </div>
        )}
      </center>
