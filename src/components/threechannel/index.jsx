...
const ChannelCardDesktop = ({ each, index, ctaEnabled }) => {
  return (
    <div data-aos="fade-up" data-aos-delay={150 * index}>
      ...
      {ctaEnabled && (
        <Link href={each.cta_link}>
          <div className={styles.lightbluebtn}>{each.cta_text} &rarr;</div>
        </Link>
      )}
    </div>
  );
...
const ChannelCardMobile = ({ each, ctaEnabled }) => {
  return (
    <>
      <div data-aos="fade-up" className={styles.thinline}></div>
      <h4 data-aos="fade-up" className={`pt-2 pt-md-0 ${styles.titletext}`}>
        {each.title}
      </h4>
      <p data-aos="fade-up" className="py-2">
        {each.description}
      </p>
...
const ThreeChannel = ({ data, ctaEnabled = true }) => {
  ...
  return (
    <Container className="my-5 py-3">
      <Row className="d-none d-md-flex">
        {data.map((each, key) => (
          <Col
            data-shono={key == currentHighlight}
      ...
      <Row className="d-block d-md-none">
        {data.map((each, key) => (
          <Col md={4} key={key} className={styles.channelmobile}>
            <ChannelCardMobile each={each} ctaEnabled={ctaEnabled} />
          </Col>
        ))}
      </Row>
    </Container>
