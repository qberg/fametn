...
export const getAllOtherAwardsFromStrapi = async (language) => {
  const maxPageSize = 100;
  const path = "other-awards?populate=deep&pagination[pageSize]=" + maxPageSize;
...
export const getAllOtherAwards = async (language) => {
  // if cache is empty, get all resources and return
  if (allOtherAwards[language].length === 0) {
    allOtherAwards[language] = await getAllOtherAwardsFromStrapi(language);
    lastUpdated = new Date();
    return allOtherAwards[language];
  }
