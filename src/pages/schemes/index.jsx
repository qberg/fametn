          ...
          return (
            <Col xl={4} lg={6} key={each.id}>
              <SchemeCard
                ...
                implementingAgency={
                  data.finance_scheme_implementing_agency?.data?.attributes
                    ?.name
                }
              />
            </Col>
    ...
    <Row>
      {allResults.length > 0 ? (
        allResults.map((each) => {
          const data = each.attributes;
          return (
            <Col xl={4} lg={6} md={12} key={each.id}>
              <SchemeCard
                link={pathname + "/" + data.scheme_link}
                title={data.scheme_name}
                icon={data.icon}
...
export async function getServerSideProps(context) {
  ...
  const path =
    "finance-scheme-categories?populate[finance_schemes][fields][0]=scheme_link&populate[finance_schemes][fields][1]=scheme_name&populate[finance_schemes][fields][2]=scheme_description&populate[finance_schemes][fields][3]=government&populate[finance_schemes][fields][4]=is_featured&populate[finance_schemes][populate][0]=icon&populate[finance_schemes][populate][1]=finance_scheme_implementing_agency&populate[finance_schemes][fields][5]=filter_subsidy_rate&populate[finance_schemes][fields][6]=filter_interest_rate&populate[finance_schemes][fields][7]=filter_grant_amount";
  ...
  const agencyList = agencyData?.data
    ?.map((agency) => agency.attributes.name)
    .sort();
...
const NoResult = () => {
  ...
  return (
    <div className={styles.noreswrap}>
...
function searchAndRank(schemes, searchText) {
  const sortByName = (firstData, secondData) =>
    firstData.attributes.scheme_name > secondData.attributes.scheme_name
      ? 1
      : -1;
  if (searchText == "") {
  ...
  const rankingFunction = (scheme) => {
    ...
    const agency =
      data.finance_scheme_implementing_agency?.data?.attributes?.name.toLowerCase();

    const score = searchWords.reduce((acc, word) => {
      return (
        acc +
        (name.includes(word) ? 10 : 0) +
        (description.includes(word) ? 5 : 0) +
        (government.includes(word) ? 3 : 0) +
        (agency.includes(word) ? 2 : 0)
      );
    }, 0);

    return score;
  };
  const ranks = schemes.map((scheme) => ({
    scheme: scheme,
    rank: rankingFunction(scheme),
  }));
  const filteredRanks = ranks.filter((rank) => rank.rank > 0);

  const sortedRanks = filteredRanks.sort(
    (first, second) => second.rank - first.rank,
  );
...
export default function Schemes({
  ...
  useEffect(() => {
    const filterSubsidyRate = (scheme) => {
      const currentSubsidyRate =
        parseInt(scheme?.attributes?.filter_subsidy_rate) || 0;
      return (
        currentSubsidyRate >= subsidyRate[0] &&
        currentSubsidyRate <= subsidyRate[1]
      );
    };

    const filterInterestRate = (scheme) => {
      const currentInterestRate =
        parseInt(scheme?.attributes?.filter_interest_rate) || 0;
      return currentInterestRate <= interestRate;
    };

    const filterGrant = (scheme) => {
      const currentGrant =
        parseInt(scheme?.attributes?.filter_grant_amount) || 0;
      return currentGrant >= grant;
    };

    const filterAgency = (scheme) => {
      const currentAgency =
        scheme?.attributes?.finance_scheme_implementing_agency?.data?.attributes
          ?.name || "No Name";
      return agencySelection.includes(currentAgency);
    };

    const requiredSchemes = filterDuplicateIds(
  ...
  const featuredSchemes = finalSchemes.filter(
    (item) => item.attributes.is_featured == true,
  );
  ...
  const pagedReqSchemes = finalSchemes.slice(
        ...
        <div className="d-flex mt-4">
          <div className={`d-none d-lg-block ${styles.leftPaneDesktop}`}>
            ...
            <Link onClick={() => setCategory(null)} href="#">
              <div
                className={`${styles.bolder} ${styles.rounded} ${category ? styles.inactive : styles.active}`}
              >
            ...
            {categories.map((each, id) => {
              return (
                <div key={id}>
                  <Link
                    onClick={() => setCategory(each.link)}
                    href={`${pathname}?category=${each.link}`}
                  >
                    <div
                      className={`${styles.rounded} ${category && category == each.link ? styles.active_opt : styles.inactive_opt} d-flex`}
                    >
                      <div>
                  ...
                  <div
                    className={`${styles.leftcontent} ${category && category == each.link ? styles.expand : styles.contract}`}
                  >
                    {each.schemes.map((scheme) => {
                      return (
                        <div
                          key={scheme.id}
                          className={`${styles.side_child} small`}
                        >
                          <Link
                            href={`${pathname}/${scheme.attributes.scheme_link}`}
                          >
                            <div className="d-flex">
          ...
          <div className={styles.fscheme}>
            ...
            {searchText == "" && (
              <>
                <FeaturedSchemesBanner schemeMeta={schemeMeta} />
